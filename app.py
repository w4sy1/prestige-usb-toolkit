...
def verify(root):
    ...
    new=sorted(actual-set(data['files']))
...
def build():
    ...
    p.add_argument('command',nargs='?',choices=['prepare','verify']);p.add_argument('--destination');p.add_argument('--tool',action='append',default=[]);p.add_argument('--apply',action='store_true')
    return p
